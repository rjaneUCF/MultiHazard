...
The package possesses the functionaity to compute the desired isoline by interpolating joint return periods calculated over a user-defined grid. This approach can result in smoother curves than overlaying the partial isolines [@Maduwantha:2024]. Also coded in the package are the methods proposed in @Barltrop:2023 to derive isolines using the @Heffernan:2004 and Wadsworth and Tawn (2013) models along with the novel bootstrap procedure for calculating sample uncertainties.
