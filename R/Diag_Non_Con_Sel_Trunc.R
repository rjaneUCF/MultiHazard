...
#' @param Selected Charactor vector of length one specifying the chosen distribution, options are the Birnbaum-Saunders \code{"BS"}, exponential \code{"Exp"}, gamma \code{"Gam"}, inverse Gaussian \code{"InvG"}, lognormal \code{"LogN"}, Tweedie \code{"Twe"} and Weibull \code{"Weib"}.
...
Diag_Non_Con_Sel_Trunc<-function(Data,x_lab,y_lim_min=0,y_lim_max=1,Selected){
  ...
  plot(0,xlim=c(0,8),ylim=c(min(0,AIC.BS,AIC.Exp,AIC.Gamma,AIC.InverseNormal,AIC.logNormal,AIC.TNormal,AIC.Tweedie,AIC.Weib),max(0,AIC.BS,AIC.Exp,AIC.Gamma,AIC.InverseNormal,AIC.logNormal,AIC.TNormal,AIC.Tweedie,AIC.Weib)),type='n',xlab="Probability Distribution",ylab="AIC",xaxt='n',cex.axis=1,cex.lab=1,las=1)
  axis(1,seq(0.5,7.5,1),c("Birn-S","Exp","Gam","InvG","LogN","TNorm","Twe","Weib"),cex.axis=0.71)
  colors <- rep("white",9)
  dist<-c("Birn-S","Exp","Gam","InvG","LogN","NA","TNorm","Twe","Weib")
  ...
  rect(3.25,0,3.75,AIC.InverseNormal,col=colors[4])
  rect(4.25,0,4.75,AIC.logNormal,col=colors[5])
  rect(5.25,0,5.75,AIC.TNormal,col=colors[7])
  rect(6.25,0,6.75,AIC.Tweedie,col=colors[8])
  rect(7.25,0,7.75,AIC.Weib,col=colors[9])
  ...
  if(Selected=="InvG"){
  fit<-fitdist(Data, "invgauss", start = list(mean = 5, shape = 1))
  lines(x,dinvgauss(x,coef(fit)[[1]],coef(fit)[[2]]),col=mypalette[4],lwd=2)
  }

  if(Selected=="LogN"){
  fit<-fitdistr(Data,"lognormal")
  lines(x,dlnorm(x,fit$estimate[1],fit$estimate[2]),col=mypalette[5],lwd=2)
  }

  if(Selected=="TNorm"){
  fit<-fitdistr(Data, "normal")
  lines(x,dtruncnorm(x,a=min(Data),mean=fit$estimate[1],sd=fit$estimate[2]),col=mypalette[7],lwd=2)
  }

  if(Selected=="Twe"){
  fit <- tweedie.profile(Data ~ 1,
                         p.vec=seq(1.5, 2.5, by=0.2), do.plot=FALSE)
  lines(x,dtweedie(x,  power=fit$p.max, mu=mean(Data), phi=fit$phi.max),col=mypalette[8],lwd=2)
  }

  if(Selected=="weib"){
  fit<-fitdistr(Data,"weibull")
  lines(x,dweibull(x,fit$estimate[1],fit$estimate[2]),col=mypalette[9],lwd=2)
  }
  ...
  if(Selected=="InvG"){
  fit<-fitdist(Data, "invgauss", start = list(mean = 5, shape = 1))
  lines(x,pinvgauss(x,coef(fit)[[1]],coef(fit)[[2]]),col=mypalette[4],lwd=2)
  }

  if(Selected=="LogN"){
  fit<-fitdistr(Data,"lognormal")
  lines(x,plnorm(x,fit$estimate[1],fit$estimate[2]),col=mypalette[5],lwd=2)
  }

  if(Selected=="Tnorm"){
    fit<-fitdistr(Data,"normal")
    lines(x,ptruncnorm(x,a=min(Data),mean=fit$estimate[1],sd=fit$estimate[2]),col=mypalette[9],lwd=2)
  }

  if(Selected=="Twe"){
    fit <- tweedie.profile(Data ~ 1,
                           p.vec=seq(1.5, 2.5, by=0.2), do.plot=FALSE)
    lines(x,ptweedie(x,  power=fit$p.max, mu=mean(Data), phi=fit$phi.max),col=mypalette[8],lwd=2,pch=16,ylab="P(X<x)")
  }

  if(Selected=="weib"){
  fit<-fitdistr(Data, "weibull")
  lines(x,pweibull(x,fit$estimate[1],fit$estimate[2]),col=mypalette[9],lwd=2)
  }
}
