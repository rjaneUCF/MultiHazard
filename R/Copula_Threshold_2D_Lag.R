...
#' @param Lag_Backward_Var1 Numeric vector of length one specifying the negative lag applied to variable in the first column of \code{Data_Detrend}. Default \code{1}.
#' @param Lag_Forward_Var1 Numeric vector of length one specifying positive lag applied to variable in the first column of \code{Data_Detrend}. Default \code{1}.
#' @param Lag_Backward_Var2 Numeric vector of length one specifying negative lag applied to variable in the second column of \code{Data_Detrend}. Default \code{1}.
#' @param Lag_Forward_Var2 Numeric vector of length one specifying  positive lag applied to variable in the second column of \code{Data_Detrend}. Default \code{1}.
...
#' Analogous vector \code{Kendalls_Tau2}, \code{p_value_Var2}, \code{N_Var2} and \code{Copula_Family_Var2} for the specified thresholds when the dataset is conditioned on the variable in column 2.
...
Copula_Threshold_2D_Lag<-function(Data_Detrend,Data_Declust,u1=seq(0.9,0.99,0.01),u2=seq(0.9,0.99,0.01),PLOT=TRUE,Lag_Backward_Var1=1,Lag_Forward_Var1=1,Lag_Backward_Var2=1,Lag_Forward_Var2=1,x_lim_min=NA,x_lim_max=NA,y_lim_min=-1,y_lim_max=1,Upper=NA,Lower=NA,GAP=0.05,Legend=TRUE,Cex_Legend=1,Cex_Axis=1,Cex_Axis_Original=1){
  ...
  if(PLOT==TRUE){
    if(is.na(u1[1])==FALSE & is.na(u2[1])==FALSE){
      ...
      mtext(round(quantile(na.omit(Data_Detrend[,2]),u2),2),at=u2,side=3,line=1,col="Red",cex=Cex_Axis_Original)
