...
#'
#' Time (in years) for a specified change in sea level according to various sea level projections. Contained within the function are three scenarios for Key West in the Southeast Florida Regional Climate Change Compact, those for Miami Beach in "Global and Regional Sea Level Rise Scenarios for the United States" NOAA et al. (2017) and those in the Interagency Sea Level Rise Scenario Tool (NOAA et al. 2022) for Naples and Miami Beach. Users can also input scenarios of their choice.
#'
...
#' @param Location Character vector of length one, specifying the location associated with the scenarios. Projections for \code{"Miami Beach"} and \code{"Naples"} are contained within the package. If a user specified scenarios are employed, set to the name of the site. Default is \code{"Key West"}.
...
#' @return For \code{"Compact"} and \code{"NOAA2022"} a list length of time for \code{SeaLevelRise} of sea level rise is expected to arise under the \code{High}, \code{Intermediate} and \code{Low}. For user specified scenarios, the time for \code{SeaLevelRise} to occur under each is returned as \code{SLR_Year}. Upper panel: A plot of the scenarios. Scenarios are in bold until the time the SeaLevelRise is reached and are transparent thereafter. Lower panel: A plot showing the number of years before is expected to occur.
#' @examples
#' #Calculate the estimated time required for 0.45m of SLR in Key West according to the scenarios in the Southeast Florida Regional Climate Change Compact
#' SLRScenarios(0.45)
#' #Calculate the estimated time required for 0.8 inches of SLR in Naples according to the scenarios in the 2022 Interagency Sea Level Rise Scenario Tool
#' SLRScenarios(0.45,Scenario="NOAA2022", Unit = "Inches", Location="Naples")
#' #Read in the scenarios for Fort Myers downloaded from https://sealevel.nasa.gov/task-force-scenario-tool/?psmsl_id=1106
...
#' #i.e. first column years, following columns the scenarios most to least extreme, converted from millimeters to meters
#' SeaLevelRise.2022_input<-data.frame(Year=seq(2020,2150,10),"High"=as.numeric(SeaLevelRise.2022[14,-(1:5)])/1000,"Medium"=as.numeric(SeaLevelRise.2022[8,-(1:5)])/1000,"Low"=as.numeric(SeaLevelRise.2022[2,-(1:5)])/1000)
#' #Calculate the estimated time required for 0.8 inches of SLR at Fort Myers
#' SLR_Scenarios(SeaLevelRise=0.8, Scenario="Other", Unit = "m", Year=2022, Location="Fort Myers", New_Scenario=SeaLevelRise.2022_input)
