...
SLR_Scenarios<-function(SeaLevelRise, Scenario="Compact", Unit = "m", Year=2022, Location="Key West", New_Scenario=NA){
  ...
  y_axis<-ifelse(Unit=="m",paste('Projected sea level rise at', Location, '(m)'),paste('Projected sea level rise at', Location,'(ft)'))
  ...
  if(Scenario=="Compact" | Scenario=="NOAA2017" | Scenario=="NOAA2022"){
    ...
    if(Low>=2100){
      text(spline.low$x[max.low]+1.55,0.25,paste('>',2100-Year),cex=1.5,font=3)
    } else{
