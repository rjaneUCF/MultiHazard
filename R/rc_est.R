...
#' rc_est(data=S22.Detrend.df.extended[,1:3],q=0.985,rp=100,mu=365.25,n_sim=100,
...
for(j in 1:n_boot){
 ...
 gpd_x$rate = length(data_x[,1][data_x[,1]>thresh_x])/length(na.omit(data_x[,1])/365.25)
 
 thresh_y = quantile(boot_df[,3],q,na.rm=T)
 gpd_y = evm(data_y[,2],th = thresh_y)
 gpd_y$rate = length(data_y[,2][data_y[,2]>thresh_y])/length(na.omit(data_y[,2])/365.25)
