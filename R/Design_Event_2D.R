...
if(Marginal_Dist1=="BS"){
  cop.sample1.non.con<-qbisa(sample[,1], as.numeric(Coef(marginal_non_con1)[1]), as.numeric(Coef(marginal_non_con1)[2]))
}
if(Marginal_Dist1=="Exp"){
...
cop.sample2.con<-u2gpd(sample[,2], p = 1, th=quantile(na.omit(Data[,con2]),Thres2) , sigma=exp(GPD_con2$coefficients[1]),xi= GPD_con2$coefficients[2])
...
sample.AND<-sample(1:length(prediction[prediction>0]),size = N_Ensemble, prob=prediction[prediction>0])
