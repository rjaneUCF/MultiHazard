...
Design_Event_2D<-function(Data, Data_Con1, Data_Con2, Thres1, Thres2, Copula_Family1, Copula_Family2, Marginal_Dist1, Marginal_Dist2, Con1="Rainfall",Con2="OsWL",mu=365.25, RP,x_lab="Rainfall (mm)",y_lab="O-sWL (mNGVD 29)",x_lim_min = NA,x_lim_max = NA,y_lim_min = NA,y_lim_max = NA,delta=0.0001,N,N_Ensemble){
  ...
  #Copula object 1
  u<-expand.grid(seq(0.0001,0.9999,delta),seq(0.0001,0.9999,delta))
  ...
  x<- seq(0.0001,0.9999,delta)
  y<- seq(0.0001,0.9999,delta)
  ...
  for(i in 1:length(seq(0,max(con1.prediction.points.ALL[,con1]),0.01))){
    j<-seq(0,max(con1.prediction.points.ALL[,1]),0.01)
  ...
  #points(cop.sample[,1],cop.sample[,2],col=rev(heat.colors(150))[20:120][1+100*((prediction-min(prediction))/(max(prediction)-min(prediction)))],lwd=3,pch=16,cex=1.75)
