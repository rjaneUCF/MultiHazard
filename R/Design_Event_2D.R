...
Design_Event_2D<-function(Data, Data_Con1, Data_Con2, Thres1, Thres2, Copula_Family1, Copula_Family2, Marginal_Dist1, Marginal_Dist2, Con1="Rainfall",Con2="OsWL",mu=365.25, RP,x_lab="Rainfall (mm)",y_lab="O-sWL (mNGVD 29)",x_lim_min = NA,x_lim_max = NA,y_lim_min = NA,y_lim_max = NA,N,N_Ensemble){ 
  ...
  #Copula object 1
  u<-expand.grid(c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))),c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))))
  ...
  x<- c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))) #seq(1*10^(-8),1-(1*10^(-8)),10^(-8))[seq(1,10^8,10^4)]
  y<- c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))) #seq(1*10^(-8),1-(1*10^(-8)),10^(-8))[seq(1,10^8,10^4)]
  ...
  u<-expand.grid(c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))),c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))))#expand.grid(c(0.00001,seq(0.0001,0.9999,0.0001)),c(0.00001,seq(0.0001,0.9999,0.0001)))
  
  #OsWL
  u1<-BiCopCDF(u[,1], u[,2], obj2)
  x<- c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))) #c(0.00001,seq(0.0001,0.9999,0.0001))
  y<- c(0.00001,seq(999.9*10^(-5),1-(1*10^(-6)),100*10^(-5))) #c(0.00001,seq(0.0001,0.9999,0.0001))
  ...
  x<-seq(0,max(con1.prediction.points.ALL[,1],con2.prediction.points.ALL[,1]),0.01)
  ...
  prediction.points.ALL_1<-data.frame(x,y)[-1,]

  y<-seq(0,max(con1.prediction.points.ALL[,2],con2.prediction.points.ALL[,2]),0.01)
  ...
  x<-numeric(length(y))
  for(i in 1:length(y)){
    x[i]<-max(con1.prediction.points.ALL[,1][which(con1.prediction.points.ALL.Round==y[i])],
              con2.prediction.points.ALL[,1][which(con2.prediction.points.ALL.Round==y[i])])
  }
  ...
  if(length(which(is.na(x)==T))>0){
    y<-y[-which(is.na(x)==TRUE)]
    x<-x[-which(is.na(x)==TRUE)]
  }
  
  prediction.points.ALL_2<-data.frame(x,y)[-1,]
  
  prediction.points.ALL<-rbind(prediction.points.ALL_1,prediction.points.ALL_2)
  ...
  points(prediction.points.ALL$Discharge,prediction.points.ALL$Surge,col="Pink")
  ...
  x.MostLikelyEvent.AND<-as.numeric(prediction.points.ALL[which(prediction==max(prediction)),][1])
  y.MostLikelyEvent.AND<-as.numeric(prediction.points.ALL[which(prediction==max(prediction)),][2])
  points(x.MostLikelyEvent.AND,y.MostLikelyEvent.AND,pch=18,cex=1.75)
  
  x.full.dependence<-max(x)
  y.full.dependence<-max(y)
