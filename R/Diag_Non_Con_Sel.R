...
#' @param x_lab Numeric vector of length one specifying. Label on the x-axis of histogram and cumulative distribution plot.
...
#' @param Selected Character vector of length one specifying the chosen distribution. Options are: Gaussian \code{"Gaus"}, Gumbel \code{"Gum"}, Laplace \code{"Lapl"}, logistic \code{"Logis"}, and reverse Gumbel \code{"RGum"}.
#' @return Panel consisting of three plots. Upper plot: Plots depicting the AIC of the two fitted distributions. Middle plot: Probability Density Functions (PDFs) of the \code{selected} distributions superimposed on a histogram of the data. Lower plot: Cumulative distribution function (CDFs) of the \code{selected} distribution overlaid on a plot of the empirical CDF.
...
Diag_Non_Con_Sel<-function(Data, Omit=NA, x_lab = "Data",y_lim_min = 0,y_lim_max = 1,Selected){
  ...
  #Second omit vector for distributions where parameter estimation fails
  Omit.2 = rep(NA,3)
  ...
  #Distributions to test
  if(any(!is.na(Omit.2))){
    Test<-Test[-which(Dist.2 %in% Omit.2[!is.na(Omit.2)])]
  }
  ...
  plot(0,xlim=c(0,length(Test)),ylim=c(min(0,AIC.Gaus,AIC.Gum,AIC.Lapl,AIC.Logis,AIC.RGum,na.rm=T),max(0,AIC.Gaus,AIC.Gum,AIC.Lapl,AIC.Logis,AIC.RGum,na.rm=T)),type='n',xlab="Probability Distribution",ylab="AIC",xaxt='n',cex.axis=1,cex.lab=1,las=1)
  axis(1,seq(0.5,length(Test)-0.5,1),c("Gaussian","Gumbel","Laplace","Logistic","Rev. Gumbel")[Test],cex.axis=0.71)
  if(any(Test==1)){rect(which(Test==1)-0.5-length(Test)/40,0,which(Test==1)-0.5+length(Test)/40,AIC.Gaus,col=ifelse(Selected=="Gaus", mypalette[3], "white"))}
  if(any(Test==2)){rect(which(Test==2)-0.5-length(Test)/40,0,which(Test==2)-0.5+length(Test)/40,AIC.Gum,col=ifelse(Selected=="Gum", mypalette[2], "white"))}
  if(any(Test==3)){rect(which(Test==3)-0.5-length(Test)/40,0,which(Test==3)-0.5+length(Test)/40,AIC.Lapl,col=ifelse(Selected=="Lapl", mypalette[4], "white"))}
  if(any(Test==4)){rect(which(Test==4)-0.5-length(Test)/40,0,which(Test==4)-0.5+length(Test)/40,AIC.Logis,col=ifelse(Selected=="Logis", mypalette[6], "white"))}
  if(any(Test==5)){rect(which(Test==5)-0.5-length(Test)/40,0,which(Test==5)-0.5+length(Test)/40,AIC.RGum,col=ifelse(Selected=="RGum", mypalette[1], "white"))}

  color = mypalette[c(3,2,4,6,1)][which(c("Gaus","Gum","Lapl","Logis","RGum") %in% Selected)]
  ...
  legend("topright",paste("Fitted",Selected,"dist."),lty=1,col=color,cex=0.9,bty='n')
  ...
  x<-seq(min(0, min(Data)), max(Data), length.out = 1000)
  ...
  legend("bottomright",c("95% Conf. Interval",paste("Fitted",Selected, "dist.")),lty=c(2,1),col=c(1,color),cex=1,bty='n',border = "white")
