...
#'
#' Fits eight (tuncated) non-extreme marginal distributions to a dataset and returns three plots demonstrating their relative goodness of fit.
#'
...
Diag_Non_Con_Trunc<-function(Data,x_lab,y_lim_min=0,y_lim_max=1){
  ...
  plot(0,xlim=c(0,8),ylim=c(min(0,AIC.BS,AIC.Exp,AIC.Gamma,AIC.InverseNormal,AIC.logNormal,AIC.TNormal,AIC.Tweedie,AIC.Weib),max(0,AIC.BS,AIC.Exp,AIC.Gamma,AIC.InverseNormal,AIC.TNormal,AIC.logNormal,AIC.Tweedie,AIC.Weib)),type='n',xlab="Probability Distribution",ylab="AIC",xaxt='n',cex.axis=1,cex.lab=1,las=1)
  ...
  rect(3.25,0,3.75,AIC.InverseNormal,col=mypalette[4])
  rect(4.25,0,4.75,AIC.logNormal,col=mypalette[5])
  rect(5.25,0,5.75,AIC.TNormal,col=mypalette[7])
  rect(6.25,0,6.75,AIC.Tweedie,col=mypalette[8])
  rect(7.25,0,7.75,AIC.Weib,col=mypalette[9])
  ...
  lines(x, dtruncnorm(x, a = min(Data), mean = fit$estimate[1],
                        sd = fit$estimate[2]), col = mypalette[7], lwd = 2)
