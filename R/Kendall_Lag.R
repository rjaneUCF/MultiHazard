...
correlation<-function(Data,lag){
  for(i in 2:(ncol(Data))){
    Data[,i]<-Lag(Data[,i],-lag[i-1])
  }
  Data<-na.omit(Data)
  return(cor(Data[2:ncol(Data)],method = "kendall")[which(lower.tri(cor(Data[2:ncol(Data)]))==T)])
}
...
for(i in 1:length(Lags)){
  Var1_Var2[i]<-correlation(Data,c(Lags[i],0,0))[1]
  Var2_Var3[i]<-correlation(Data,c(0,0,Lags[i]))[2]
  Var1_Var3[i]<-correlation(Data,c(0,0,Lags[i]))[3]
}

correlation.test<-function(Data,lag){
  for(i in 2:(ncol(Data))){
    Data[,i]<-Lag(Data[,i],-lag[i-1])
  }
  Data<-na.omit(Data)
  return(cor.test(Data[,2],Data[,3],method = "kendall")$p.value)
}
...
for(i in 1:length(Lags)){
  Var1_Var2_Test[i]<-correlation.test(Data[,-4],c(Lags[i],0))
  Var2_Var3_Test[i]<-correlation.test(Data[,-3],c(0,Lags[i]))
  Var1_Var3_Test[i]<-correlation.test(Data[,-2],c(0,Lags[i]))
}
