...
#' @section Details:
#' EDF-statistics are goodness-of-fit statistics based on a comparison of the Empirical Distribution Function (EDF) \eqn{F_n} and a candidate parametric probability distribution \eqn{F} [@Stephens1974]. Quadratic EDF test measure the distance between \eqn{F} and \eqn{F_n} by:\deqn{n\int^{\infty}_{-\infty}=(F(x)-F_n(x))^2w(x) dx}
#' where \eqn{n} is the number of elements in the original sample and \eqn{w(x)} is a weighting function.
#' The Cramer Von Misses statistic \eqn{w(x)=1}, whereas the Anderson-Darling (AD) statistic \eqn{A^2}, assigns more weight to the tails of the data by setting \eqn{w(x)=\frac{1}{F(x)(1-F(x))}}. Under the null hypothesis that the sample \eqn{x_1,\dots,x_n} is from a GPD, the transformation \eqn{z=F-1(x)} a sample z uniformly distribution between \eqn{0} and \eqn{1}.  \deqn{A^2=-\frac{1}{n}\sum_{i=1}^{n} \{(2i-1)[log(z_i)+log(1-z_{n+z-i})]\}-n}
...
#' \deqn{u_0 = \argminA_{u_j} (1-p(u_j)).}
#' @return List comprising \itemize{
#' \item \code{u_Candidate}
...
#' \item \code{GPD_MLE}
#' GPD parameter estimates, Mean Residual Life Plot (MRLP) values and return level estimates associated with each \code{u_Candidate}.
#' \item \code{CI_Upper}
...
GPD_Threshold_Solari<-function(Event,Data,RPs=c(10,50,100,500,1000),RPs_PLOT=c(2,3,4),Min_Quantile=0.95,Alpha=0.1,mu=365.25,N_Sim=10){
  ...
  # POT with L-moments and with bootstrapping c.i.'s
  Event = na.omit(Event)
  ...
  for(i in 1:N_u_Candidate){
    ...
    y <- rep(p_val$NDATA,6)
    m<-max(1,min((1+(round(GPD.MLE[i],4)*10^4)),length(p_val$AU2VAL))) #length(p_val$AU2VAL)-
    ...
    #print(paste('Testing a threshold of',u_Candidate[i]))
  }
  ...
  res<-list("u_Candidate"=u_Candidate,"u_Candidate_Quantile"=u_Candidate_Quantile,"GPD_MLE"=GPD.MLE,"CI_Upper"=CI.Upper,"CI_Lower"=CI.Lower,"AR2"=AR2,"AR2_pValue"=AR2.pValue,"Candidate_Thres"=Candidate_Thres)
  return(res)
}
