...
Standard_Copula_Fit<-function(Data,Copula_Type="Gaussian"){
  ...
  Model <- normalCopula(c(as.numeric(coef(Gaussian_Copula_Fit)[1:(ncol(Data))])),dim=ncol(Data), dispstr="un")
  }
  ...
  if(Copula_Type=="tcopula"){
    ...
    Model <- tCopula(c(as.numeric(coef(t_Copula_Fit)[1:(ncol(Data))])),dim=ncol(Data), dispstr="un",df=as.numeric(coef(t_Copula_Fit)[(ncol(Data))]))
  }
