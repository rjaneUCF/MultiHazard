...
#' @param u Numeric vector of length one specifying the threshold to analyze, chosen by the user based on plots from the \code{GPD_Threshold_Solari} function.
...
#' @param N_Sim Numeric vector of length one specifying the size of the samples used in the bootstrap simulation. Default is \code{10^4}.
...
#' \item \code{xi}
#' GPD shape parameter estimate for the threshold is \code{u}.
#' \item \code{sigma}
#' GPD scale parameter estimate for the threshold is \code{u}.
#' \item \code{u}
#' GPD location parameter estimate for the threshold is \code{u}.
#' \item \code{rate}
#' GPD rate parameter i.e., number of independent excesses per year for a threshold of \code{u}.
#' \item The remaining columns are \code{RL}
#' Return level estimates from the GPD using a threshold of \code{u}.
#' }
...
#' Top row: Histograms of the GPD parameter estimates based on a nonparametric bootstrapping simulation. Grey bars correspond to the estimates obtained as the threshold (\code{u}) is varied, found by running the function a necessary input of the function. Continuous black lines correspond to results obtained by fixing the threshold at \code{u}. Dashed blue lines correspond to the expected values for the fixed threshold.
#' Lower left: Return level plot. Return levels of the observations estimated from the empirical distribution. Grey bars correspond to the maximum of the upper and lower bounds of the \eqn{100(1-\frac{Alpha}{2})\%} confidence intervals as the threshold is varied. Continuous black lines correspond to results obtained by fixing the threshold at \code{u}. Dashed blue lines correspond to the expected values for the fixed threshold.
...
#'
GPD_Threshold_Solari_Sel<-function(Event,Data,Solari_Output,u,Alpha=0.1,N_Sim=10^4,RP_Max=1000,RP_Plot=100,mu=365.25,y_lab="Data"){
  ...
  plot(log10(1/((1-(1:length(Exceedence))/(length(Exceedence)+1))*Rate)),sort(Event[which(Event>u)]),
  ...
  points(log10(1/((1-(1:length(Exceedence))/(length(Exceedence)+1))*Rate)),sort(Event[which(Event>u)]),col="Green",pch=16)
  ...
  names(Estimate)<-c("xi","sigma","u","rate",as.character(RP))
  res<-list(Estimate=Estimate,CI_Lower=CI.Lower,CI_Upper=CI.Upper)
  return(res)
}
