...
Plot.RI.evm<-function(Data,model,min.RI=1,max.RI=1000,y.lab="Return Level",main.RI="",Cex.axis=1,Cex.lab=1){
  ...
  for(i in 1:length(seq(min.RI,max.RI,0.1))){
    RI.Estimate[i]<-rl(model, M = seq(min.RI,max.RI,0.1)[i],ci.fit=TRUE)[[1]][1]
    RI.Lower[i]<-rl(model, M = seq(min.RI,max.RI,0.1)[i],ci.fit=TRUE)[[1]][2]
    RI.Upper[i]<-rl(model, M = seq(min.RI,max.RI,0.1)[i],ci.fit=TRUE)[[1]][3]
  }
  ...
  points(log10((length(Data) + 1) / ((1:length(Data) * model$rate)))[1:length(which((length(Data) + 1) / ((1:length(Data) * model$rate))>min.RI))],
         rev(sort(Data))[1:length(which((length(Data) + 1) / ((1:length(Data) * model$rate))>min.RI))],pch=16)
}
