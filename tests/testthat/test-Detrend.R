...
test_that("Linear method works", {
  test_data <-S22_T_MAX_Daily_Completed_Detrend[,c(1,3)]  
  ...
  # Check return  length
  expect_length(result, nrow(S22_T_MAX_Daily_Completed_Detrend))
...
test_that("Window method works", {
  ...
  expect_false(identical(result, S22_T_MAX_Daily_Completed_Detrend[,4] ))
})
...
test_that("Invalid inputs produce errors", {
  expect_error(Detrend(S22_T_MAX_Daily_Completed_Detrend[,c(1,3)], Method = "invalid"))
})
