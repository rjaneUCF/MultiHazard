...
test_that("Gaussian copula works", {
  ...
  expect_equal(length(params), ncol(S20.Detrend.df)*(ncol(S20.Detrend.df)-1)/2)
})
...
test_that("t-copula works", {
  ...
  expect_equal(length(params), 1+ ncol(S20.Detrend.df)*(ncol(S20.Detrend.df)-1)/2)
...
test_that("Invalid inputs produce errors", {
  ...
  expect_error(Standard_Copula_Fit(Copula_Type="Gumbel"),
               "Error: Data is empty")
